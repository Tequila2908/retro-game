...
export default class GameController {
  ...
  nextLevel() {
    this.currentMove = 'user';
    if (this.level === 1) {
      this.userTeam = Teams.getStartUserTeam();
      this.enemyTeam = generateTeam(Teams.getEnemyTeam(), 1, 2);
      this.addPositionCharacter(this.userTeam, this.enemyTeam);
    } else if (this.level === 2) {
      this.currentTheme = themes.desert;
      this.userTeam = generateTeam(Teams.getUserTeam(), 1, 1);
      this.enemyTeam = generateTeam(
        Teams.getEnemyTeam(), 2, (this.userTeam.length + userPositions.length),
      );
      this.addPositionCharacter(this.userTeam, this.enemyTeam);
    } else if (this.level === 3) {
      this.currentTheme = themes.arctic;
      this.userTeam = generateTeam(Teams.getUserTeam(), 2, 2);
      this.enemyTeam = generateTeam(
        Teams.getEnemyTeam(), 3, (this.userTeam.length + userPositions.length),
      );
      this.addPositionCharacter(this.userTeam, this.enemyTeam);
    } else if (this.level >= 4) {
    // } else if (this.level === 4) { Task 18: Time Killer implemented
      this.currentTheme = themes.mountain;
      this.userTeam = generateTeam(Teams.getUserTeam(), 3, 2);
      this.enemyTeam = generateTeam(
        Teams.getEnemyTeam(), 4, (this.userTeam.length + userPositions.length),
      );
      this.addPositionCharacter(this.userTeam, this.enemyTeam);
    } else {
      this.blockedBoard = true;
      GamePlay.showMessage(`You won!!! Your score is ${this.point} points.`);
      return;
    }
  ...
  loadGame() {
    try {
      ...
      console.log(e);
      GamePlay.showMessage('Downloading failed...');
      this.newGame();
    }
  }
    ...
    if (!this.blockedBoard) {
      for (const item of [...userPositions, ...enemyPositions]) {
        if (item.position === index) {
          this.gamePlay.showCellTooltip(getLifeBar(item.character), index);
        }
      }
  ...
  async characterAttacker(attacker, target) {
    ...
    if (targetedCharacter.health <= 0) {
      ...
      if (userPositions.length === 0) {
        GamePlay.showMessage('Sorry, you lost that game...');
        this.blockedBoard = true;
      }
      if (enemyPositions.length === 0) {
        for (const item of userPositions) {
